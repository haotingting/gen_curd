...
public class #DOMAIN_NAME#Controller extends WaterBootBaseController {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public WaterBootResultBean<#DOMAIN_NAME#Ro> queryList(String json) {
        WaterBootResultBean<#DOMAIN_NAME#Ro> result = new WaterBootResultBean<#DOMAIN_NAME#Ro>();
        try {
            boolean flag = true;
            String msg = null;
            if (null != json && !"".equals(json)) {
                ...
                if(flag){
                    ...
                    if(count.longValue() > 0){
                        ...
                        List<#DOMAIN_NAME#Ro> #DOMAIN_NAME_LOWER#List = #DOMAIN_NAME_LOWER#Service.find#DOMAIN_NAME#List(so);
                        result.setCount(count);
                        result.setRst(#DOMAIN_NAME_LOWER#List);
                    }
                    result.setCode(WaterBootResultContext.ResultCode.SUCCESS.getCode());
                    result.setMsg("查询成功");
                }
